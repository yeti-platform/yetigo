...
from yetigo.transforms.utils import get_yeti_connection, get_av_sig, \
    get_hash_entities, get_status_domains, get_sample_by_ip_vt, \
    get_hostnames_by_ip_vt, \
    get_ips_by_hostname_vt, run_oneshot, get_observable
...
class VTIPassiveDNS(Transform):
    ...
    def do_transform(self, request, response, config):
        ...
        for r in res['links']:
            ...
            context_vt = [(entity.value, c[entity.value]) for c in
                          obs['context']
                          if c['source'] == 'VirusTotal PDNS'
                          and entity.value in c
                          ]
